...
const EmployeeStarred = () => {
  ...
  const fetchEmails = useCallback(async (page = 1) => {
    try {
      ...
      const response = await emailAPI.getStarred(params);
    ...
    return (
      <EmployeeLayout>
        <div className="flex items-center justify-center h-[calc(100vh-180px)]" role="status" aria-live="polite">
          ...
          <span className="sr-only">Loading starred emails</span>
        </div>
      </EmployeeLayout>
    );
  }
        ...
        <div className="px-4 md:px-6 py-3 md:py-4 border-b border-gray-200 flex-shrink-0">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap items-center space-x-2 md:space-x-3">
              ...
              <h2 className="text-lg md:text-xl font-semibold text-navy">Starred</h2>
              <span className="text-sm text-gray-500" aria-label={`${totalCount} starred emails`}>({totalCount})</span>
            </div>
            <button
              onClick={() => setShowSearch(!showSearch)}
              className="p-1.5 md:p-2 text-gray-500 hover:text-navy rounded-lg hover:bg-gray-100"
              aria-label={showSearch ? 'Close search' : 'Search starred emails'}
            >
          ...
          {showSearch && (
            <div className="mt-3">
              <input
                ...
                autoFocus
                aria-label="Search starred emails"
              />
            </div>
          )}
        </div>
          ...
          <div className="divide-y divide-gray-100">
            {emails.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 md:py-16 text-gray-500">
                ...
                <p className="text-base md:text-lg font-medium">No starred emails</p>
                <p className="text-xs md:text-sm">Star emails to access them quickly</p>
              </div>
            ) : (
              emails.map((email) => (
                <div
