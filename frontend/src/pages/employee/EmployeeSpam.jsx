...
import { FiAlertCircle, FiCheckCircle, FiSearch, FiChevronLeft, FiChevronRight } from 'react-icons/fi';
...
const EmployeeSpam = () => {
  ...
  const fetchEmails = useCallback(async (page = 1) => {
    try {
      ...
      const response = await emailAPI.getSpam(params);
      ...
      toast.error('Failed to load spam emails');
    } finally {
      setLoading(false);
    }
  }, [searchQuery]);
  ...
  const handleNotSpam = async (emailId) => {
    try {
      await emailAPI.toggleSpam(emailId);
      toast.success('Email removed from spam');
    ...
    return (
      <EmployeeLayout>
        <div className="flex items-center justify-center h-[calc(100vh-180px)]" role="status" aria-live="polite">
          ...
          <span className="sr-only">Loading spam emails</span>
        </div>
      </EmployeeLayout>
    );
  }
        ...
        <div className="px-4 md:px-6 py-3 md:py-4 border-b border-gray-200 flex-shrink-0">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap items-center space-x-2 md:space-x-3">
              <FiAlertCircle className="w-4 h-4 md:w-5 md:h-5 text-red-500" aria-hidden="true" />
              <h2 className="text-lg md:text-xl font-semibold text-navy">Spam</h2>
              <span className="text-sm text-gray-500" aria-label={`${totalCount} spam emails`}>({totalCount})</span>
            </div>
            <button
              onClick={() => setShowSearch(!showSearch)}
              className="p-1.5 md:p-2 text-gray-500 hover:text-navy rounded-lg hover:bg-gray-100"
              aria-label={showSearch ? 'Close search' : 'Search spam emails'}
            >
          ...
          <div className="divide-y divide-gray-100">
            {emails.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 md:py-16 text-gray-500">
                ...
                <p className="text-base md:text-lg font-medium">No spam emails</p>
                <p className="text-xs md:text-sm">Your spam folder is clean</p>
              </div>
            ) : (
              emails.map((email) => (
                <div
                ...
                >
                  <div className="flex-1 min-w-0 cursor-pointer" onClick={() => navigate(`/employee/email/${email.id}`)}>
                    <div className="flex flex-wrap items-center gap-1.5 md:gap-3">
                      <span className="font-medium truncate text-sm md:text-base">{email.sender_name}</span>
                      <span className="px-1.5 md:px-2 py-0.5 text-[8px] md:text-xs bg-red-100 text-red-700 rounded-full flex-shrink-0">Spam</span>
                    </div>
                  ...
                  <div className="flex items-center space-x-1.5 md:space-x-2 ml-2 md:ml-4 flex-shrink-0">
                    <button
                      ...
                      className="p-1 md:p-2 text-gray-500 hover:text-green-600 rounded-lg hover:bg-green-50"
                      aria-label="Remove from spam"
                    >
                      <FiCheckCircle className="w-3.5 h-3.5 md:w-4 md:h-4" aria-hidden="true" />
                    </button>
