...
import {
  ...
  FiSmile, FiFile, FiX, FiCheck, FiCheckCircle, FiTrash2,
  FiCornerUpLeft, FiImage, FiDownload
} from 'react-icons/fi'
...
const EmployeePrivateChat = () => {
  const { userId } = useParams()
  const navigate = useNavigate()
  const location = useLocation()
  const { user } = useAuth()
  const { socket, sendPrivateMessage, deletePrivateMessage, sendPrivateTyping, addReaction, isUserOnline, markPrivateRead } = useSocket()
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (message) => {
      if (message.sender_id === parseInt(userId) || message.receiver_id === parseInt(userId)) {
        ...
        if (message.sender_id === parseInt(userId)) {
          ...
          setTimeout(() => {
            markPrivateRead({ messageId: message.id, senderId: userId })
          }, 500)
        }
      }
    }
    ...
    const handleTyping = ({ senderId, isTyping }) => {
      if (senderId === parseInt(userId)) setContactTyping(isTyping)
    }
    ...
    socket.on('private-message-deleted', handleDeleted)
    socket.on('reaction-updated', handleReaction)
    socket.on('user-private-typing', handleTyping)
    socket.on('message-read', handleMessageRead)

    return () => {
      socket.off('new-private-message', handleNewMessage)
      socket.off('private-message-deleted', handleDeleted)
      socket.off('reaction-updated', handleReaction)
      socket.off('user-private-typing', handleTyping)
  ...
  const fetchContact = async () => {
    try {
      const users = await chatAPI.getPrivateUsers()
  ...
  const fetchMessages = async () => {
    try {
      const response = await chatAPI.getPrivateMessages(userId, { limit: 100 })
  ...
  const handleSendMessage = async (e) => {
    ...
    if (attachments.length > 0) {
      setUploading(true)
      try {
        const saved = await chatAPI.sendPrivateMessage({
        ...
        if (socket) {
          setTimeout(() => {
            markPrivateRead({ messageId: saved.id, senderId: userId })
          }, 500)
        }
      } catch (error) {
    ...
    setAttachments([])
    setReplyTo(null)
    sendPrivateTyping({ receiverId: parseInt(userId), isTyping: false })
  }
  ...
  const handleDeleteMessage = async (messageId) => {
    if (window.confirm('Delete this message?')) {
      deletePrivateMessage({ messageId, receiverId: parseInt(userId) })
      setSelectedMessage(null)
    }
  }

  const handleReaction = (messageId, emoji) => {
    addReaction({ messageId, emoji, receiverId: parseInt(userId) })
    setShowEmojiPicker(null)
  }
  ...
  const handleTyping = (e) => {
    ...
    sendPrivateTyping({ receiverId: parseInt(userId), isTyping: true })
  ...
  const getMessageStatus = (message) => {
    if (message.is_read) {
      return <FiCheckCircle className="text-emerald-500 w-3 h-3 md:w-3.5 md:h-3.5" />
    }
    return <FiCheck className="text-gray-400 w-3 h-3 md:w-3.5 md:h-3.5" />
  }

  const online = contact ? isUserOnline(contact.id) : false
    ...
    return (
      <EmployeeLayout>
        <div className="flex flex-col items-center justify-center h-full text-slate-500 p-4">
          <p className="text-base md:text-lg font-medium">User not found</p>
          <button
            onClick={() => navigate('/employee/chat')}
            className="mt-3 md:mt-4 bg-emerald-500 text-white px-4 md:px-6 py-1.5 md:py-2 rounded-full hover:bg-emerald-600 transition-colors text-sm md:text-base"
          >
            Back to chats
          </button>
        </div>
        ...
        <div className="bg-white border-b border-slate-200 px-3 md:px-4 py-2 md:py-3 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center space-x-2 md:space-x-3 min-w-0">
            ...
            <div className="relative flex-shrink-0">
              <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center text-white font-semibold text-xs md:text-sm">
                {contact?.full_name?.charAt(0)?.toUpperCase() || 'U'}
              </div>
              {online && (
                <div className="absolute bottom-0 right-0 w-2.5 h-2.5 md:w-3 md:h-3 bg-emerald-500 rounded-full border-2 border-white"></div>
              )}
            </div>
            <div className="min-w-0">
              <p className="font-semibold text-slate-800 text-sm md:text-base truncate">{contact?.full_name || 'User'}</p>
              <p className="text-[10px] md:text-xs text-slate-400 truncate">
                {contactTyping ? <span className="text-emerald-500">typing...</span> : online ? 'Online' : 'Offline'}
              </p>
            </div>
          </div>
          <button className="p-1.5 md:p-2 hover:bg-slate-100 rounded-full transition-colors flex-shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-2.5 sm:p-3 md:p-4 space-y-1.5 bg-slate-50">
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-400 p-4 text-center">
              ...
              <p className="text-xs md:text-sm">Say hello to {contact?.full_name || 'this user'}</p>
            </div>
          ) : (
                ...
                >
                  <div className={`max-w-[88%] sm:max-w-[80%] md:max-w-[75%] flex flex-col ${isOwn ? 'items-end' : 'items-start'}`}>
                    {isReply && parentMessage && (
                      <div className="mb-1 px-2 md:px-3 py-1 md:py-1.5 rounded-lg text-[10px] md:text-xs border-l-4 border-emerald-500 bg-slate-100 max-w-[180px] sm:max-w-[220px] md:max-w-sm">
        ...
        {replyTo && (
          <div className="bg-white border-t border-slate-200 px-3 md:px-4 py-1.5 md:py-2 flex items-center justify-between flex-shrink-0">
            <div className="flex-1 flex items-center gap-1.5 md:gap-2 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-[10px] md:text-xs font-medium text-emerald-600">Replying to {replyTo.sender_name}</p>
...
export default EmployeePrivateChat