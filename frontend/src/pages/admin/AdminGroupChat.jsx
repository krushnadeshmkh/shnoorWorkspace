...
import {
  ...
  FiUsers, FiLogOut, FiImage, FiDownload, FiUserPlus
} from 'react-icons/fi'
...
const AdminGroupChat = () => {
  ...
  const { user } = useAuth()
  const { socket, sendMessage, deleteMessage, sendTyping, addReaction, joinGroup, isUserOnline } = useSocket()
      ...
      <div className="flex flex-col flex-1 min-h-0 bg-slate-50 rounded-xl overflow-hidden border border-slate-200">
        <div className="relative flex flex-1 min-h-0 overflow-hidden">
          <div className="flex-1 flex flex-col min-w-0">
            ...
            {!isMember ? (
              <div className="flex flex-col items-center justify-center flex-1 bg-slate-50 p-4 text-center">
                <div className="text-4xl md:text-6xl mb-3 md:mb-4">🔒</div>
                <p className="text-base md:text-lg font-medium text-gray-700">You are not a member</p>
                <p className="text-xs md:text-sm text-gray-400 mb-4 md:mb-6">Join the group to start chatting</p>
                <button
                  onClick={handleJoinGroup}
                  className="bg-emerald-500 text-white px-4 md:px-6 py-2 md:py-2.5 rounded-full font-medium hover:bg-emerald-600 transition-colors flex items-center gap-2 text-sm md:text-base"
                >
                    ...
                    return (
                      <div key={member.id} className="flex items-center px-2 md:px-3 py-1.5 md:py-2 rounded-lg hover:bg-slate-50">
                        <div className="relative flex-shrink-0">
                          <div className="w-8 h-8 md:w-9 md:h-9 rounded-full bg-gradient-to-br from-indigo-400 to-indigo-600 flex items-center justify-center text-white text-xs md:text-sm font-semibold">
                            {member.full_name?.charAt(0)?.toUpperCase() || 'U'}
                          </div>
                          {isOnline && (
                            <div className="absolute bottom-0 right-0 w-2 h-2 md:w-2.5 md:h-2.5 bg-emerald-500 rounded-full border-2 border-white"></div>
                          )}
                        </div>
