...
import { FiMessageSquare, FiUsers, FiSearch, FiUser, FiX, FiClock, FiFilter, FiCalendar, FiFile, FiPaperclip, FiPlus, FiMoreVertical } from 'react-icons/fi'
...
const AdminChat = () => {
  ...
  const loadSearchHistory = () => {
    try {
      const history = JSON.parse(localStorage.getItem('adminChatSearchHistory') || '[]')
  ...
  const saveSearchHistory = (query) => {
    ...
    try {
      ...
      localStorage.setItem('adminChatSearchHistory', JSON.stringify(updated))
  ...
  const handleGroupClick = (groupId, messageId = null) => {
    ...
    setSelectedIndex(-1)
    if (messageId) {
      navigate(`/admin/chat/group/${groupId}?messageId=${messageId}`)
    } else {
      navigate(`/admin/chat/group/${groupId}`)
    }
  }
  ...
  const handlePrivateChat = (userId, messageId = null) => {
    ...
    setSelectedIndex(-1)
    if (messageId) {
      navigate(`/admin/chat/private/${userId}?messageId=${messageId}`)
    } else {
      navigate(`/admin/chat/private/${userId}`)
    }
  }
      ...
      <div className="flex flex-col md:flex-row h-[calc(100vh-12rem)] md:h-[calc(100vh-10rem)] bg-slate-50 rounded-xl overflow-hidden border border-slate-200 relative">
        <div className="w-full md:w-80 lg:w-96 border-r border-slate-200 bg-white flex flex-col min-h-0 relative z-10">
          <div className="px-4 md:px-5 py-3 md:py-4 border-b border-slate-100 flex items-center justify-between flex-shrink-0">
            ...
            <button
              onClick={() => navigate('/admin/chat/create-group')}
              className="p-1.5 md:p-2 hover:bg-slate-100 rounded-full transition-colors"
              title="New group"
            >
              <FiPlus className="w-4 h-4 md:w-5 md:h-5 text-emerald-600" />
            </button>
          </div>
            ...
            {showHistory && searchHistory.length > 0 && !search.trim() && (
              <div className="absolute left-0 right-0 top-full mt-1 bg-white rounded-xl shadow-xl border border-slate-200 max-h-[200px] overflow-y-auto z-50">
                ...
                <div className="px-4 py-1.5 border-t border-slate-100 bg-slate-50 rounded-b-xl text-center">
                  <button
                    onClick={() => {
                      localStorage.removeItem('adminChatSearchHistory')
                      setSearchHistory([])
                      setShowHistory(false)
                    }}
                ...
                {activeTab === 'chats' ? (
                  ...
                  <>
                    <div
                      ...
                      className="flex items-center px-3 md:px-4 py-2.5 md:py-3 hover:bg-slate-50 cursor-pointer transition-colors"
                    >
                      <div className="w-9 h-9 md:w-11 md:h-11 rounded-full bg-emerald-50 flex items-center justify-center flex-shrink-0">
                        <FiPlus className="w-4 h-4 md:w-5 md:h-5 text-emerald-600" />
                      </div>
                      <span className="ml-2 md:ml-3 font-medium text-emerald-600 text-xs md:text-sm">Create new group</span>
                    </div>
                    {filteredGroups.length === 0 ? (
                      <div className="text-center py-12 md:py-16 text-slate-400">
                        <FiUsers className="w-8 h-8 md:w-10 md:h-10 mx-auto mb-2 md:mb-3 opacity-40" />
                        <p className="text-xs md:text-sm font-medium">No groups yet</p>
                      </div>
                    ) : (
