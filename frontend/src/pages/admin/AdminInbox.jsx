...
const handleSend = async (e) => {
  ...
  if (loading && emails.length === 0) {
    return (
      <AdminLayout>
        <div className="flex items-center justify-center h-[calc(100vh-200px)]" role="status" aria-live="polite">
  ...
  return (
    <AdminLayout>
      <div className="bg-white rounded-xl shadow-card overflow-hidden flex flex-col relative" style={{ height: 'calc(100vh - 120px)' }}>
                  ...
                  >
                    ...
                    >
                      {selectedEmails.includes(email.id) ? (
                        <FiCheckSquare className="w-4 md:w-5 h-5 text-blue-600" aria-hidden="true" />
                      ) : (
                        <FiSquare className="w-4 md:w-5 h-5" aria-hidden="true" />
                      )}
                    </button>
