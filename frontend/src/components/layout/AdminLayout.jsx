...
import { 
  FiHome, FiUsers, FiMessageSquare, FiMail, FiActivity, 
  FiSettings, FiLogOut, FiUser, FiBell,
  FiMessageCircle, FiInbox, FiEdit, FiSend, FiFileText, 
  FiArchive, FiAlertCircle, FiTrash2, FiStar, FiTag,
  FiChevronDown, FiChevronRight, FiPlus, FiMoreHorizontal,
  FiMenu, FiX
} from 'react-icons/fi'
...
const AdminLayout = ({ children }) => {
  ...
  const adminNavItems = [
    ...
    { path: '/admin/users', icon: FiUsers, label: 'Employees' },
    { path: '/admin/groups', icon: FiMessageSquare, label: 'Groups' },
    { path: '/admin/emails', icon: FiMail, label: 'Email Monitoring' },
    { path: '/admin/chat-monitor', icon: FiMessageCircle, label: 'Chat Monitoring' },
    { path: '/admin/activity', icon: FiActivity, label: 'Activity Logs' },
  ]

  const emailNavItems = [
    { path: '/admin/inbox', icon: FiInbox, label: 'Inbox' },
    { path: '/admin/sent', icon: FiSend, label: 'Sent' },
    { path: '/admin/drafts', icon: FiFileText, label: 'Drafts' },
    { path: '/admin/archive', icon: FiArchive, label: 'Archive' },
    { path: '/admin/spam', icon: FiAlertCircle, label: 'Spam' },
    { path: '/admin/trash', icon: FiTrash2, label: 'Trash' },
    { path: '/admin/important', icon: FiStar, label: 'Important' },
    { path: '/admin/starred', icon: FiStar, label: 'Starred' },
  ]
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-64 bg-navy text-white flex flex-col transform transition-transform duration-300 ease-in-out ${
        ...
        <div className="p-4 md:p-6 border-b border-white/10 flex items-center justify-between">
          <div>
            ...
            <p className="text-white/50 text-[10px] md:text-xs mt-0.5">Admin Panel</p>
          </div>
          <button
            ...
            <div className="flex items-center space-x-2 md:space-x-4">
              ...
              <div className="flex items-center space-x-1.5 md:space-x-2">
                <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-navy/10 flex items-center justify-center" aria-hidden="true">
                  <span className="text-navy font-semibold text-xs md:text-sm">{user?.fullName?.charAt(0) || 'A'}</span>
                </div>
